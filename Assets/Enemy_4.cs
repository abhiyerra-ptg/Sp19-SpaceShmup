...
public class Part
{
    //These three fields need to be defined in the INspector pan
    ...
    [HideInInspector] //Make field on the next line not appear in the Inspector
...
public class Enemy_4 : Enemy {
    ...
    private float duration = 4; //Duration of the movement

	// Use this for initialization
	void Start () {
        //There is already an initial position chosen by Main.SpawnEnemy()
        //so add it to points as the the initial p0 & p1
        p0 = p1 = pos;
        InitMovement();
            ...
            case "ProjectileHero":
                ...
                foreach(Part prt in parts)
                {
                    if (!Destroyed(prt))
                    {
                        ...
                        allDestroyed = false;
                        //...change allDestroyed to false
                        break;
                        //Break out of the foreachloop
                    }
                }
                if (allDestroyed)
                {
                    //If it IS completely destroyed
                    //...tell the Main singleton that htis ship was destroyed
